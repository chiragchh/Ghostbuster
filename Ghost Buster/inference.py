...
#        util.raiseNotDefined()
        # P(g_t|O_1:t-1, O_t) propertional to P(g_t|O_1:t-1)*P(O_t|g_t)
#        print(noisyDistance)
#        print(emissionModel)
...
#            print("Jail position = ", self.getJailPosition())
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
#        util.raiseNotDefined()
        #P(g_t+1 |O_1:t) = sum_g_t { P(g_t, g_t+1 | O_1:t)}
        #                = sum_g_t { P(g_t| O_1:t) P(g_t+1|g_t)}
        allPossible = util.Counter()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        # selecting given number of particles evenly
        count = len(self.legalPositions)
        for i in range(self.numParticles):
            particles.append(self.legalPositions[i%count])
...
#        print self.getBeliefDistribution()
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter() # a container to store new beliefs
        belief = self.getBeliefDistribution() # old beliefs
        for particle in belief:
            trueDistance = util.manhattanDistance(particle, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                if noisyDistance is None:
                    # handling condition when ghost is caught
                    allPossible[particle] = 0
                else:
                    # updating belief based on evidence
                    allPossible[particle] = belief[particle] * emissionModel[trueDistance]
        if noisyDistance is None:
#            print("Jail position = ", self.getJailPosition())
            allPossible[self.getJailPosition()] = 1
            # Updating belief such that ghost is in prision cell when caught
            self.particles = [self.getJailPosition()]
        else:
            totalCount = allPossible.totalCount()
            if totalCount == 0:
                # condition when all particles are eliminated and new initialization should be done
                self.initializeUniformly(gameState)
            else:
                allPossible.normalize()
                # using provided util to sample new particles based on normalized newly learned belief
...
#            print "prior beliefs::", self.getBeliefDistribution()
        jointInference.addGhostAgent(self.ghostAgent)

    def observeState(self, gameState):
        "Update beliefs based on the given distance observation and gameState."
        if self.index == 1:
#            print "old beliefs::", self.getBeliefDistribution()
            jointInference.observeState(gameState)
#            print "new beliefs::", self.getBeliefDistribution()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        count = len(positions)
        for i in range(self.numParticles):
            particles.append(positions[i%count])
#        print "num particles = ", self.numParticles
        self.particles = particles
#        print self.getBeliefDistribution()
    ...
    def observeState(self, gameState):
        ...
        allPossible = util.Counter() # a container to store new beliefs
        for particle in self.particles: #assumption: particles capture belief
            likelihood = 1.0
            for i in range(self.numGhosts):
...
#                if emissionModel[trueDistance] > 0:
            ...
            # updating belief based on evidence
            allPossible[particle] += likelihood
                        
        totalCount = allPossible.totalCount()
        if totalCount == 0:
            # condition when all particles are eliminated and new initialization should be done
            self.initializeParticles()
            # handling case for captured ghosts
            for index in range(self.numParticles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None:
                        self.particles[index] = self.getParticleWithGhostInJail(self.particles[index], i)
        else:
            allPossible.normalize()
                # using provided util to sample new particles based on normalized newly learned belief
            items = sorted(allPossible.items())
